...
import { Wrench, Disc, ArrowRight, User as UserIcon, UserPlus, Shield, ClipboardCheck } from 'lucide-react';
...
interface MenuSeleccionProps {
  onSelectOption: (opcion: 'mantencion' | 'neumaticos' | 'crear_usuario' | 'mecanico' | 'perfil') => void;
  onLogout?: () => void;
}

export default function MenuSeleccion({ onSelectOption }: MenuSeleccionProps) {
  ...
  const rol = currentUser?.rol?.toUpperCase() || 'CONDUCTOR';
      ...
      <main className="menu-main-content">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black bg-blue-100 text-blue-800 px-2 py-0.5 rounded-full border border-blue-300 flex items-center gap-1">
                  <Shield size={11} className="text-blue-600" /> Rol: {rol}
                </span>
              </div>
        ...
        <div className="menu-welcome-text">
          ...
          <p className="menu-welcome-subtitle">
            Seleccione un módulo según su rol asignado ({rol}) para ingresar o gestionar registros.
          </p>
        </div>

        <div className="menu-options-grid">
          {/* Módulo Conductor: Solicitud de Mantención Taller */}
          <div
          ...
          {/* Módulo Conductor: Reporte de Neumáticos */}
          <div
          ...
          {/* Módulo Supervisor: Crear Nuevos Usuarios */}
          {isSupervisorOrAdmin && (
            <div
            ...
            >
              <div>
                ...
                <h3 className="menu-card-title">Registrar Nuevo Usuario</h3>
                <p className="menu-card-desc">
                  Crear credenciales para choferes, supervisores y mecánicos (POST /api/v1/auth/register).
                </p>
              </div>
