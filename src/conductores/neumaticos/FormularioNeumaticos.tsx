...
import { enviarReporteNeumatico } from "./neumaticosService";
import ModalResumenNeumatico from "./ModalResumenNeumatico";
import { formatearMonedaChilena, limpiarSoloNumeros } from "../../utils/formatters";
import { validarMaquinaBus } from "../../utils/validators";
...
export default function FormularioNeumaticos({
  ...
  // Formato de precio chileno ($ XX.XXX) con utilidad centralizada
  const manejarPrecio = (e: React.ChangeEvent<HTMLInputElement>) => {
    setPrecio(formatearMonedaChilena(e.target.value));
  };

  // Formato Marca de Fuego (Solo números)
  const manejarMarcaFuego = (e: React.ChangeEvent<HTMLInputElement>) => {
    setMarcaFuego(limpiarSoloNumeros(e.target.value));
  };
  ...
  const handlePreSubmit = (e: React.FormEvent) => {
    ...
    const valBus = validarMaquinaBus(maquina);
    if (!valBus.valido) {
      setErrorMsg(valBus.error || "⚠️ OBLIGATORIO: Debes ingresar el número de máquina.");
      return;
    }
  ...
  // Envío final al backend usando el servicio desacoplado
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      const resData = await enviarReporteNeumatico({
        usuario_id: usuarioId,
        maquina: busFormatted,
        tipo_bus: tipoBus,
        ruedas: ruedasString,
        motivo: motivoFinal,
        precio: precioLimpio || undefined,
        marca_fuego: marcaFuego.trim() || undefined,
        evidencia: foto,
      });

      const recordData = (resData as any)?.data || resData || {};
      const recordId =
        recordData.id ||
        resData.reporte_id ||
        (resData as any).id ||
        (resData as any).folio ||
      ...
      const detail = getApiErrorMessage(err, "No se pudo enviar la solicitud al servidor.");
      setErrorMsg(`No se pudo procesar el reporte: ${detail}`);
      setShowConfirmModal(false);
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <div className="fn-wrapper font-sans text-slate-900">
      ...
      <ModalResumenNeumatico
        ...
        maquina={maquina}
        chofer={chofer}
        ruedasSeleccionadas={ruedasSeleccionadas}
        motivo={motivo}
        otroMotivo={otroMotivo}
        precio={precio}
        marcaFuego={marcaFuego}
        hasFoto={Boolean(fotoPreview)}
      />
    </div>
  );
}
