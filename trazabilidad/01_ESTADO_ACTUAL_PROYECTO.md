...
| :--- | :--- | :---: | :--- |
| **Enrutamiento & Navegación** | `src/App.tsx`<br>`src/components/TopBar/TopBar.tsx` | `COMPLETADO` | Enrutamiento declarativo por URL (`/home`, `/perfil`, `/mantencion`, `/neumaticos`, `/mecanico`, `/supervision`, `/crear_usuario`, `/login`). Protección de rutas con guardia de sesión. Navbar superior minimalista centrada en la marca corporativa `NARBUS` sin botones de distracción. |
...
| **Módulo Mecánicos: Pauta Preventiva (11 Ítems)** | `src/mecanicos/revisiones/PautaPreventivaModal.tsx` | `COMPLETADO` | Modal de inspección de los 11 ítems maestros de taller con estados `OK`, `DEFECTO` y `NO_APLICA`, observaciones obligatorias ante defecto y guardado en bloque (`POST /pauta`). |
...
| **Módulo Supervisores: Auditoría Inmutable** | `src/supervisores/supervision/DashboardSupervision.tsx`<br>`src/supervisores/supervision/tabs/ModalDetalleAuditoria.tsx` | `COMPLETADO` | Tablero de trazabilidad histórica con filtros por N° de bus, estado y nombre de mecánico. Ficha completa de auditoría mediante ventana modal flotante de alta ergonomía con despliegue de historial de mecánicos activos vs relevos, fallas con banderas de repuesto faltante, tiempos de atención, evidencias fotográficas y justificaciones operacionales. |
